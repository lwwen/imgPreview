		...
		imgPreview: function(opts) {
			$(this).each(function(index, ele) {
				...
				*/
				 ...
				 *   选择多张图片上传时创建多个div
				 */
				var createDiv = function(dom, files, type) {
					for (var n = 0; n < files.length; n++) {
						var _self = $(dom).parents('div').children('div');
						console.log(_self)
						var divIndex = $(dom).parent('div').index();
						$(dom).parents('.img-preview-wrap').prepend(wrapInnerHtml);
						var _newDiv = $(dom).parents('div').children('div').eq(0);
						setWrap(_newDiv)
						if (type == 'back-clip') {
				...
				$(ele).find('input[type="file"]').change(function(e) {
					 ...
					 */
					$(this).wrap('<form>').closest('form').reset();
					$(this).unwrap('<form>');
					/**
					 * --------------------------------------------------
					 *
